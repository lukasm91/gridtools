    ...
    def generate(self, out_file):
        ...
        for mss_id, (mss, mss_stage_analysis) in enumerate(
            ...
            intervals = [
                ...
                for stage in stages
                for interval in self.computation.stages[stage].intervals
            ]
            levels = set(
            ...
            for begin, end in zip(levels[:-1], levels[1:]):
                        ...
                        for i in self.computation.stages[stage_ref.name].intervals:
                            ...
                            if interval.contains(first):
                                overload = (
                                    interval
                                    if i.overload
                                    == interface_pb2.StageInterval.INTERVAL
                                    else "none"
                                )
                                d_data.append(
                                    {
                                        "name": stage_ref.name.replace(
                                            "(anonymous namespace)::", ""
                                        ),
                                        "id": stage_id,
                                        "overload": overload,
                ...
                mss_data["intervals"].append(
                    {"interval": (first, last), "stages": i_data}
                )
            ...
            for d, d_stage_extents in zip(mss.dependent_stages, mss_stage_analysis):
                d_data = []
                for stage_ref, stage_extent in zip(
                    d.independent_stages, d_stage_extents
                ):
                    s_data = {}

                    s_data["stage_extent"] = stage_extent
                    s_data["name"] = stage_ref.name.replace(
                        "(anonymous namespace)::", ""
                    )
                    s_data["id"] = stage_id
                    ...
                    for arg, accessor in zip(stage_ref.args, stage.accessors):
                        if arg.arg_type == interface_pb2.Multistage.NORMAL:
                            ...
                            arg_type = "temporary"
                        s_data["argmap"].append(
                            {"arg": (arg_type, arg.id), "accessor": accessor.id}
                        )
            ...
            max_stage_extent = reduce(
                lambda a, b: max_extent(a, reduce(max_extent, b)),
                mss_stage_analysis,
                (0, 0, 0, 0),
            )
                ...
                for stage_ref in d.independent_stages:
                    ...
                    for arg_ref, accessor in zip(stage_ref.args, stage.accessors):
                        if arg_ref.arg_type == interface_pb2.Multistage.TEMPORARY:
                            if arg_ref.id not in mss_data["temporaries"]["args"]:
                                mss_data["temporaries"]["args"][arg_ref.id] = {
                                    "type": self.computation.temporaries[
                                        arg_ref.id
                                    ].type
                                }
                            ...
                            if (
                                "readonly"
                                not in mss_data["temporaries"]["args"][arg_ref.id]
                                or mss_data["temporaries"]["args"][arg_ref.id][
                                    "readonly"
                                ]
                            ):
                            ...
                            if arg.kind not in mss_data["kinds"]:
                                mss_data["kinds"][arg.kind] = {}
                                mss_data["kinds"][arg.kind]["args"] = {}

                            if arg_ref.id not in mss_data["kinds"][arg.kind]["args"]:
                            ...
                            if (
                                "readonly"
                                not in mss_data["kinds"][arg.kind]["args"][arg_ref.id]
                                or mss_data["kinds"][arg.kind]["args"][arg_ref.id][
                                    "readonly"
                                ]
                            ):
                    ...
                    args = args | set(
                        [(arg.arg_type, arg.id) for arg in stage_ref.args]
                    )
            ...
            for k_cache in mss.k_caches:
                ...
                if not k_cache.temporary:
                    mss_data["k_caches"][-1]["kind"] = self.computation.fields.args[
                        k_cache.id
                    ].kind
                    if k_cache.fill or k_cache.flush:
                        mss_data["k_caches"][-1]["readonly"] = mss_data["kinds"][
                            self.computation.fields.args[k_cache.id].kind
                        ]["args"][k_cache.id]["readonly"]
                    mss_data["kinds"][self.computation.fields.args[k_cache.id].kind][
                        "args"
                    ][k_cache.id]["cached"] = "K"
                    mss_data["kinds"][self.computation.fields.args[k_cache.id].kind][
                        "args"
                    ][k_cache.id]["local"] = (not k_cache.fill and not k_cache.flush)
                else:
                    if k_cache.fill or k_cache.flush:
                        mss_data["k_caches"][-1]["readonly"] = mss_data["temporaries"][
                            "args"
                        ][k_cache.id]["readonly"]

                    mss_data["temporaries"]["args"][k_cache.id]["cached"] = "K"
                    mss_data["temporaries"]["args"][k_cache.id]["local"] = (
                        not k_cache.fill and not k_cache.flush
                    )
            ...
            for ij_cache in mss.ij_caches:
                ...
                if not ij_cache.temporary:
                    mss_data["kinds"][self.computation.fields.args[ij_cache.id].kind][
                        "args"
                    ][ij_cache.id]["cached"] = "K"
                else:
        ...
        env.filters.update(
            {
                ...
                "masked": lambda k, i: k["layout"][i] == -1
                if "layout" in k
                else k["selector"][i],
                "unitstride": lambda k, i: k["layout"][i] == 0
                if "layout" in k
                else not any(k["selector"][0:i]),
                "has_kcaches": lambda multistage: "k_caches" in multistage.keys()
                and len(multistage["k_caches"]) > 0,
